...
class LogicOR : public Algorithm{
    public:
        ...
        void update(){
            compute();
            gpioMapDigitalWrite(LogicOR::gate_outputs, state);
        };

        void compute(){
            uint8_t input_state[GPIO_N] = {0};
            gpioMapDigitalRead(LogicOR::gate_inputs, &input_state[0]);
...
class LogicNOR : public LogicOR{
    public:
        LogicNOR(uint8_t midi_inputs, uint8_t midi_outputs, uint16_t gate_inputs, uint16_t gate_outputs) :
            LogicOR(midi_inputs, midi_outputs, gate_inputs, gate_outputs){};
        void update() {
            compute();
            gpioMapDigitalWrite(LogicOR::gate_outputs, !state);
        };
