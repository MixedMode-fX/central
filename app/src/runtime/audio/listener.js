...
export class Listener {
  ...
  // whatever arrives at index 3 next. The rule is `renumberNodeKeys`, shared
  // with the canvas positions, which move for the same reason.
  forgetDrumNode(index) {
    const kept = renumberNodeKeys(this.drums, index);
    const surviving = new Set(kept.values());
    for (const voice of this.drums.values()) if (!surviving.has(voice)) voice.gain?.disconnect();
    for (const [key, voice] of kept) voice.key = key;
    this.drums = kept;
  }
