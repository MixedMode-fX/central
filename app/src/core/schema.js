...
// descriptors. No DOM, so `app/test/app.test.mjs` runs them against the real
// firmware: it validates every example patch against the generated schema and
// rejects a patch the firmware would reject, which is what stops the schema
// drifting into a description of a machine this is not. The page that hands
// them over is `ui/tabs/SchemaTab.js`.
...
// comes from the firmware's own grouping, the same way the node card decides
