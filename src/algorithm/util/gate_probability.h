...
// same beats and either one's `decision` can be read by the other.
//
...
// own: it is high for exactly as long as an input gate whose edge
...
//
// It says in one outlet what `NOT(decision)` and an `AND` with this node's
// own input already say in two nodes and a bus, which is normally the
// argument against having it. It is here because the pair is one rule asked
// of two signals (algorithm/util/trig_condition.h): the note side cannot be
// built from cables at all - nothing downstream can tell a dropped note-on
// from one that was never played - and an outlet that exists on one half and
// not the other would make the two nodes two algorithms. `decision` is
// untouched and is still what a *neighbour* rule reads: it is another node's
// answer, which no outlet here can carry.
//
...
//         own length.
// Outlet 2 (gate): decision - this node's last answer, latched. `AND` it
