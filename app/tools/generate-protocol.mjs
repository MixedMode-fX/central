...
// emits the JavaScript, and `make app` fails if what is checked in differs
// from what the headers say. A protocol change therefore breaks both builds at
// once, which was the whole reason for keeping the app in this repository.
//
...
const constants = {
  ...
  ...defines(configH, [
    ...
    'N_CC_MAP', 'N_MOD_ROUTE', 'N_MACRO', 'N_MACRO_DEST', 'N_MACRO_DEST_PER_MACRO', 'MACRO_NAME_BYTES', 'MASTER_PPQN', 'CLOCK_SUBTICK', 'CLOCK_MIN_BPM', 'CLOCK_MAX_BPM',
    'CLOCK_DEFAULT_BPM', 'MAX_SEQUENCE_LEN', 'NOTE_SEQ_VOICES', 'DRUM_SEQ_LANES',
    'MONITOR_EVENTS', 'MONITOR_MAX_NODES', 'MONITOR_NODE_VALUES',
  ]),
  ...
  ...defines(read('src/node/param.h'), ['PARAM_CENTRE', 'ENV_TIME_UNIT_US']),
};
...
// scale ("minor") where the parameter block holds a 12-bit mask, and a
// hand-copied table of masks is exactly the kind of thing that rots quietly
// into a sequencer playing the wrong notes.
function scaleMasks(source, ids) {
  const text = strip(source);
  const body = text.match(/uint16_t\s+scale_mask\([^)]*\)\s*\{([\s\S]*?)\n\}/);
  if (!body) throw new Error('scale_mask() not found');
  const masks = {};
  for (const [, name, value] of body[1].matchAll(/case\s+(\w+)\s*:\s*return\s+(0b[01]+|0x[0-9a-fA-F]+|\d+)\s*;/g)) {
    if (!(name in ids)) throw new Error(`scale_mask returns for ${name}, which is not a ScaleId`);
    masks[name] = Number(value);
  }
  const fallback = body[1].match(/default\s*:\s*return\s+(0b[01]+|0x[0-9a-fA-F]+|\d+)\s*;/);
  if (!fallback) throw new Error('scale_mask() has no default');
  // Everything the switch does not name falls through to the default, which is
  // chromatic; SCALE_COUNT is the enum's size, not a scale.
  for (const name of Object.keys(ids)) {
    if (name === 'SCALE_COUNT') continue;
    masks[name] ??= Number(fallback[1]);
  }
  return masks;
}
...
const lines = [];
lines.push('// GENERATED by app/tools/generate-protocol.mjs - do not edit.');
lines.push('//');
lines.push('// The message layout is defined once, in the firmware headers, and this file');
lines.push('// is derived from them. `make app` regenerates it and fails if the checked-in');
