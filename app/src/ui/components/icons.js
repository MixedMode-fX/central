...
const frame = (...children) => svg('svg', {
  viewBox: '0 0 24 24', class: 'icon', 'aria-hidden': 'true', focusable: 'false',
}, ...children);
...
// controller plugs in here": the learn button. Circles, so it is not a path
// in the table above.
function midiIcon() {
  return frame(
    svg('circle', { cx: 12, cy: 12, r: 9, fill: 'none', stroke: 'currentColor', 'stroke-width': 1.6 }),
    ...[[6.2, 12], [7.9, 7.9], [12, 6.2], [16.1, 7.9], [17.8, 12]].map(([cx, cy]) =>
      svg('circle', { cx, cy, r: 1.6, fill: 'currentColor' })),
    svg('rect', { x: 9.8, y: 14.6, width: 4.4, height: 2.6, rx: 1.3, fill: 'currentColor' }));
}
...
export function icon(name) {
  ...
  return frame(svg('path', { d: spec.d, ...(spec.fill ? { fill: 'currentColor' } : STROKE) }));
}
