// What a control does, asked for by holding it.
//
...
import { SWAP_TIMINGS, channelLabel } from '../../protocol/names.js';
...
export function AssignSheet(app, where) {
  ...
  return el('div', { class: 'sheet', role: 'dialog', 'aria-label': 'what this control does' },
    ...
    el('div', { class: 'sheet-body' },
      // The caption is the silkscreen, so it is the first thing and the
      // biggest: it is what will be read at arm's length, and everything
      // below it is what it does.
      el('input', {
...
// A number, as two big targets either side of a readout. A native spinner's
// arrows are three pixels wide on a phone, and everything set here is set
// with a thumb.
...
function PadFields(app, where, pad, set) {
  ...
  if (pad.kind === PadKind.PROGRAM) return el('div', { class: 'stack' }, kind, Launch(app, pad, set));
  ...
  if (pad.kind === PadKind.NOTE) {
    ...
    what.push(Field({ label: 'channel' }, ChannelField(pad.channel, (channel) => set({ channel }))));
  }
...
function Launch(app, pad, set) {
  ...
  return el('div', { class: 'stack' },
    ...
    g.pcEnabled
      ? Hint(`the module recalls Program Change on ${g.pcChannel ? `channel ${g.pcChannel}` : 'any channel'}`)
      : Row(el('span', { class: 'hint' }, 'the module is not recalling Program Change'),
            el('button', {
              onclick: () => app.editor.setGlobals(
                { pcEnabled: 1, pcChannel: pad.channel }, 'Program Change recall'),
            }, 'turn recall on')));
}
